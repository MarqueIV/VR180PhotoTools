...
namespace JpegParser {
	...
	public class JpegFile {
		...
		public void WriteVr180Jpeg(Vr180Jpeg jpegs, string xmpMetadata, byte[] extendedMd5Hash, string extendedXmp, string outJpeg) {
			...
			var md5bytes = Encoding.UTF8.GetBytes(sb.ToString());
			...
			foreach (XmlNode node in xmlNodes) {
				...
				foreach (XmlAttribute attribute in node.Attributes) {
					...
					if (attribute.Name.Equals("GImage:Data")) {
						...
						var imageData = Convert.FromBase64String(attribute.Value + padding);

						// uncomment lines below to write out the embedded image. 
						//var jpegFile = new FileStream("newjpeg.jpg", FileMode.OpenOrCreate, FileAccess.Write);
						//jpegFile.Write(imageData, 0, imageData.Length);
						return imageData;
					}
				}
			}
